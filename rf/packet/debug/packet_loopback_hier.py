...
class PduDebugBlock(gr.basic_block):
    ...
    def __init__(self, label, add_seq=False):
        gr.basic_block.__init__(self, name=f"PDU Debug ({label})", in_sig=None, out_sig=None)
        self._label = label
        self._add_seq = add_seq  # If True, add sequence number to metadata
    ...
    def _get_next_seq(self):
        ...
        with PduDebugBlock._seq_lock:
            PduDebugBlock._seq_counter += 1
            return PduDebugBlock._seq_counter
    ...
    def handle_msg(self, msg):
        try:
            if pmt.is_pair(msg):
                ...
                # Extract or add sequence number
                if self._add_seq:
                    seq = self._get_next_seq()
                    # Add sequence number to metadata
                    if pmt.is_dict(meta):
                        meta = pmt.dict_add(meta, pmt.intern("pkt_seq"), pmt.from_long(seq))
                    else:
                        meta = pmt.dict_add(pmt.make_dict(), pmt.intern("pkt_seq"), pmt.from_long(seq))
                else:
                    # Extract sequence number from metadata
                    seq = 0
                    if pmt.is_dict(meta):
                        seq_pmt = pmt.dict_ref(meta, pmt.intern("pkt_seq"), pmt.PMT_NIL)
                        if seq_pmt != pmt.PMT_NIL and pmt.is_integer(seq_pmt):
                            seq = pmt.to_long(seq_pmt)
                
                if pmt.is_u8vector(data):
                    vec = list(pmt.u8vector_elements(data))
                    hex_bytes = " ".join(f"{b:02x}" for b in vec)
                    print(f"[{self._label}] pkt={seq} len={len(vec)} bytes={hex_bytes}", flush=True)
                elif pmt.is_blob(data):
                    vec = list(pmt.blob_data(data))
                    hex_bytes = " ".join(f"{b:02x}" for b in vec)
                    print(f"[{self._label}] pkt={seq} len={len(vec)} bytes={hex_bytes}", flush=True)
                else:
                    print(f"[{self._label}] pkt={seq} non-u8vector data type", flush=True)
                
                # Forward message with updated metadata
                self.message_port_pub(pmt.intern("out"), pmt.cons(meta, data))
            else:
                print(f"[{self._label}] pkt=? non-pair message", flush=True)
                self.message_port_pub(pmt.intern("out"), msg)
        except Exception as e:
            print(f"[{self._label}] pkt=? ERROR: {e}", flush=True)
            import traceback
            traceback.print_exc()
            # Forward message even on error
            self.message_port_pub(pmt.intern("out"), msg)
...
class packet_loopback_hier(gr.top_block):#, Qt.QWidget
    ...
    def __init__(self):
        ...
        # self.settings = Qt.QSettings("gnuradio/flowgraphs", "packet_loopback_hier")
        ...
        # Blocks
        ##################################################
        self.pdu_random_pdu_0 = pdu.random_pdu(20, 22, 0xFF, 2)
        ...
        # Create debug blocks at each stage
        # TX_IN adds sequence number to metadata, RX blocks extract it
        self.blocks_debug_pdu_tx = PduDebugBlock("TX_IN", add_seq=True)
        self.blocks_debug_pdu_rx = PduDebugBlock("RX_OUT", add_seq=False)
        self.blocks_debug_pdu_rxcrc = PduDebugBlock("RX_CRC", add_seq=False)

        ##################################################
        # Connections
        ##################################################
        self.msg_connect((self.blocks_message_strobe_0, 'strobe'), (self.pdu_random_pdu_0, 'generate'))
        # Log bytes going into TX (PDU) and forward
        self.msg_connect((self.pdu_random_pdu_0, 'pdus'), (self.blocks_debug_pdu_tx, 'in'))
        self.msg_connect((self.blocks_debug_pdu_tx, 'out'), (self.packet_tx_0, 'in'))

        self.connect((self.packet_tx_0, 0), (self.blocks_throttle2_0, 0))
        self.connect((self.blocks_throttle2_0, 0), (self.packet_rx_0, 0))

        # Log bytes coming out of RX (PDU)
        self.msg_connect((self.packet_rx_0, 'pkt out'), (self.blocks_debug_pdu_rx, 'in'))
        self.msg_connect((self.packet_rx_0, 'precrc'), (self.blocks_debug_pdu_rxcrc, 'in'))
