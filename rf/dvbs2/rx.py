...
MAX_RX_GAIN = {'usrp': 75, 'rtl': 50, 'bladeRF': 60, 'plutosdr': 71, 'hackrf': 62}
MAX_FREQ = {'usrp': 6e9, 'rtl': 2.2e9, 'bladeRF': 6e9, 'plutosdr': 6e9, 'hackrf': 6e9}
MIN_FREQ = {'usrp': 10e6, 'rtl': 22e6, 'bladeRF': 47e6, 'plutosdr': 70e6, 'hackrf': 1e6}
...
class DVBS2RxTopBlock(gr.top_block, Qt.QWidget):
    ...
    def setup_gui(self):
        ...
        if self.source in ['usrp', 'rtl', 'bladeRF', 'plutosdr', 'hackrf']:
            ...
            elif self.source == 'hackrf':
                # Total gain is LNA + VGA (0-40 + 0-62)
                initial_gain = self.hackrf['lna'] + self.hackrf['vga']
    ...
    def connect_source(self):
        ...
        if (self.source == "fd" or self.source == "file"):
            ...
            source = self.setup_hackrf_source()
    ...
    def connect_sink(self):
        ...
        if (self.sink == "fd"):
            ...
            # Parse IP and port from self.out_udp (format: "IP:PORT")
            if not isinstance(self.out_udp, str) or not self.out_udp:
                raise ValueError(f"--out-udp argument is required and must be a non-empty string of the format IP:PORT")
            if ':' in self.out_udp:
                udp_ip, udp_port = self.out_udp.split(':', 1)
                udp_port = int(udp_port)
            else:
                raise ValueError(f"Invalid --out-udp value '{self.out_udp}'. Expected format: IP:PORT")
                
            sink = network.udp_sink(gr.sizeof_char, 1, udp_ip, udp_port, 0, 1316, False)
    ...
    def set_gui_gain(self, new_gain):
        if self.source == 'usrp':
            ...
            # Distribute gain between LNA (0-40) and VGA (0-62)
            # Prefer VGA for higher gain values
            if new_gain <= 40:
                self.hackrf['lna'] = new_gain
                self.hackrf['vga'] = 0
            elif new_gain <= 62:
                self.hackrf['lna'] = 40
                self.hackrf['vga'] = new_gain - 40
            else:
                self.hackrf['lna'] = 40
                self.hackrf['vga'] = min(new_gain - 40, 62)
            self.hackrf_source.set_gain(0, 'LNA', self.hackrf['lna'])
            self.hackrf_source.set_gain(0, 'VGA', self.hackrf['vga'])
    ...
    def set_gui_freq(self, new_freq):
        ...
        if self.source == 'usrp':
            ...
            self.hackrf_source.set_frequency(0, self.freq)
...
def argument_parser():
    ...
    src_group.add_argument(
        "--source",
        choices=["fd", "file", "rtl", "usrp", "bladeRF", "plutosdr", "hackrf"],
        default="fd",
    ...
    snk_group.add_argument("--out-udp",
                           type=str,
                           help="Output UDP address:port used if sink=udp. Example: 127.0.0.1:2000")
    ...
    hackrf_group.add_argument('--hackrf-vga',
                              type=float,
                              default=16,
                              help="HackRF VGA gain in dB (0-62 dB, step 2 dB)")
    ...
    if options.source == "hackrf":
        if not (0 <= options.hackrf_lna <= 40):
            parser.error("--hackrf-lna must be between 0 and 40 dB")
        if not (0 <= options.hackrf_vga <= 62):
            parser.error("--hackrf-vga must be between 0 and 62 dB")
